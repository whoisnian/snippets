...
将以下代码保存为 `hybrid_encryption.mjs`，执行 `node hybrid_encryption.mjs` 运行该脚本
```js showLineNumbers
...
将以下代码保存为 `hybrid_encryption.html`，使用 Chrome 140+ 或 Firefox 133+ 打开该文件在控制台查看运行结果
...
将以下代码保存为 `hybrid_encryption.go`，执行 `go run hybrid_encryption.go` 运行该程序
```go showLineNumbers
package main
